...
class VAANetErase_binary(nn.Module):
    def __init__(self,
                 snippet_duration,
                 sample_size,
                 n_classes,
                 ...
                 audio_n_segments=10,
                  # NEW ↓↓↓
                 saliency_level='feature_map'):
        super(VAANetErase_binary, self).__init__()
        ...
        self.saliency_topk_ratio = 0.25   # 논문 스타일 시작점: top 25% keep
    ...
    def _build_topk_binary_mask_3d(self, saliency_3d: torch.Tensor, topk_ratio: float):
        """
        saliency_3d: [N, 1, T, H, W]
        return:      [N, 1, T, H, W]  (0/1 binary mask)
        """
        N = saliency_3d.size(0)
        flat = saliency_3d.reshape(N, -1)   # [N, T*H*W]
        ...
        mask_3d = mask_flat.view_as(saliency_3d)
        return mask_3d
    ...
    def forward(self, input: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            print("[DEBUG] feat shape after backbone:", tuple(feat.shape))
            print("[DEBUG] T' size =", feat.size(2))
            assert feat.size(2) == 1, f"Expected T'=1, got feat.shape={feat.shape}"
        ...
        if self.saliency_level == 'feature_map':
            ...
            saliency_resized = nn.functional.adaptive_avg_pool3d(
                saliency_map_flat, (feat.size(2), feat.size(3), feat.size(4))
            )  # [B*S, 1, T', H', W']
            ...
            binary_mask = self._build_topk_binary_mask_3d(
                S, topk_ratio=self.saliency_topk_ratio
            )  # [B*S, 1, T', H', W']

            # 채널 차원 확장
            saliency_mask = binary_mask.expand_as(feat)  # [B*S, C, T', H', W']

            # hard selection
            feat = feat * saliency_mask

            if not hasattr(self, "_printed_topk_mask"):
                print("[top-k mask] keep ratio:", binary_mask.mean().item())
                print("[top-k mask] shape:", tuple(binary_mask.shape))
                self._printed_topk_mask = True
