...
class VAANetErase_mean_dac(nn.Module):
    def __init__(self,
                 snippet_duration,
                 sample_size,
                 n_classes,
        ...
        super(VAANetErase_mean_dac, self).__init__()
        ...
        target = self._find_last_conv(self.resnet_backbone)
    ...
    def forward(self, input: torch.Tensor, target_class=None, compute_gradcam=False):
        v, a, s = input
        ...
        if compute_gradcam:
            v = self.resnet_pool(feat).flatten(1)
        else:
            with torch.no_grad():
        ...
        if not compute_gradcam:
            return output, None, None, gamma

        if self._gc_activ is None:
            raise RuntimeError("Grad-CAM activation is None.")

        if target_class is None:
            target_class = output.argmax(dim=1)

        score = output.gather(1, target_class.view(-1, 1)).sum()
        grads = torch.autograd.grad(score, self._gc_activ, retain_graph=False, create_graph=False)[0]

        A = self._gc_activ
        w = grads.mean(dim=(2, 3, 4), keepdim=True)
        cam3 = torch.relu((w * A).sum(dim=1))   # [B*S, T', H', W']
        cam2 = cam3.mean(dim=1)                 # [B*S, H', W']
        ...
        return output, None, None, gamma, cam_map
