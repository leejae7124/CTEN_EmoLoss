...
def make_dataset(frame_root_path,
    ...
    for class_name in sorted(os.listdir(subset_frame_root)):
        ...
        for video_id in sorted(os.listdir(class_dir)):
            frame_dir = os.path.join(class_dir, video_id)
            if not os.path.isdir(frame_dir):
                continue

            audio_path = os.path.join(subset_audio_root, class_name, f'{video_id}.mp3')
            saliency_dir = os.path.join(subset_saliency_root, class_name, video_id)
...
class CAERDataset(data.Dataset):
    def __init__(self,
                 video_path,
                 audio_path,
                 saliency_path,
        ...
        # CTEN-style (CAER에 맞게 수정)
        self.norm_mean = -7.355195083618164
        self.norm_std = 4.154593391257628
