from core.utils import AverageMeter, process_data_item, run_model_loss, calculate_accuracy,batch_augment,batch_augment2
import time
import os

# 원하는 A6000 UUID 2장으로 강제 (파이썬 내에서 마지막으로 덮어쓰기)
...
def train_epoch(epoch, data_loader, model, criterion, optimizer, opt, class_names, writer):
    ...
    for i, data_item in enumerate(data_loader):
        ...
        output1, loss1, gamma1 = run_model_loss(opt, [visual, target, audio,saliency_map], model, criterion, i, print_attention=False, use_intensity=True,)
        ...
        visual_erase2, sal_erase2 =batch_augment(video_item,high_index,opt,visual,saliency_map, sal_path)
        output2, loss2, gamma2 = run_model_loss(opt, [visual_erase2, target, audio, sal_erase2], model, criterion, i, print_attention=False, use_intensity=True,)
        
        visual_erase3, sal_erase3 =batch_augment2(video_item, low_index, opt, visual, saliency_map, sal_path)
        output3, loss3, gamma3 = run_model_loss(opt, [visual_erase3, target, audio, sal_erase3], model, criterion, i, print_attention=False, use_intensity=True,)
        ...
        accuracies3.update(acc3, batch_size)
