...
def train_epoch(epoch, data_loader, model, criterion, optimizer, opt, class_names, writer):
    ...
    for i, data_item in enumerate(data_loader):
        ...
        saliency_map = torch.ones_like(saliency_map)  # <- 여기! 0 대신 1   # NEW: saliency도 0
        ...
        # sal_erase2 = torch.zeros_like(sal_erase2)
        # print("run model2")
        # output2, loss2, gamma2 = run_model(opt, [visual_erase2, target, audio, sal_erase2], model, criterion, i, print_attention=False)
        # visual_erase3, sal_erase3 =batch_augment2(video_item, low_index, opt, visual, saliency_map, sal_path)
        
        # sal_erase3 = torch.zeros_like(sal_erase3) 
        # print("run model3")
        # output3, loss3, gamma3 = run_model(opt, [visual_erase3, target, audio, sal_erase3], model, criterion, i, print_attention=False)
